...
float g_eqValues[10] = {0.0f}; // Global EQ values
bool g_eqEnabled = true;

// EQ frequency bands (in Hz)
const float g_eqFrequencies[10] = {32, 64, 125, 250, 500, 1000, 2000, 4000, 8000, 16000};

// Forward declarations
void StyleTabBar();
void DrawNestedFrame(const char* title, bool rgbMode);
void EndNestedFrame(bool rgbMode);
void DrawAlignedSeparator(const char* label, bool rgbMode);
...
void CreateConfiguration(const char* file_path) {
    ...
    if (ofs) {
        ...
        bool default_isolation = false;
        bool default_hider = false;
        int default_hotkey = VK_F5;
        float default_bitrate = 510000.0f;
        ImVec4 default_main_color = ImVec4(1.0f, 0.0f, 0.0f, 1.00f);
        ImVec4 default_clear_color = ImVec4(0.0f, 0.0f, 0.0f, 1.00f);
        bool default_reverb = false;
        float default_reverb_mix = 0.5f;
        float default_reverb_size = 0.7f;
        float default_reverb_damping = 0.5f;
        float default_reverb_width = 1.0f;
        bool default_rgb_mode = false;
        float default_rgb_speed = 0.5f;
        int default_channel_mode = 1; // Default to stereo (index 1)
...
void ResetConfiguration() {
    Gain = 1.0f; // Default value for Gain
    ExpGain = 1.0f; // Default value for ExpGain
    Spoofing::ProcessIsolation = false; // Default for ProcessIsolation
    Spoofing::Hider = false; // Default for Hider
    TOGGLE_HOTKEY = VK_F5; // Default hotkey
    bitrateValue = 510000.0f; // Default bitrate (minimum)
    // Reset colors to defaults
    main_color = ImVec4(1.0f, 0.0f, 0.0f, 1.00f); // Red accent color
    clear_color = ImVec4(0.0f, 0.0f, 0.0f, 1.00f); // Black background
    reverbEnabled = false; // Default reverb disabled
    reverbMix = 0.5f; // Default mix level
    reverbSize = 0.7f; // Default room size
    reverbDamping = 0.5f; // Default damping
    reverbWidth = 1.0f; // Default width
    rgbModeEnabled = false; // Default RGB mode disabled
    rgbCycleSpeed = 0.5f; // Default RGB cycle speed
    audioChannelMode = 1; // Default to stereo (index 1)
...
void InitEQFilters() {
    // Bass filter (lowpass, cutoff ~200Hz at 48kHz sample rate)
    bassFilter.a0 = 0.018f;
    bassFilter.a1 = 0.036f;
    bassFilter.a2 = 0.018f;
    bassFilter.b1 = -1.56f;
    bassFilter.b2 = 0.64f;
    bassFilter.gain = 1.2f; // Slightly boost bass

    // Mid filter (bandpass, center ~1kHz at 48kHz sample rate)
    // Wider Q to be more musical
    midFilter.a0 = 0.10f;
    midFilter.a1 = 0.0f;
    midFilter.a2 = -0.10f;
    midFilter.b1 = -1.75f;
    ...
    // High filter (highpass, gentler slope, cutoff ~3.5kHz instead of 5kHz)
    // Gentler slope to prevent harsh S sounds
    highFilter.a0 = 0.23f;
    highFilter.a1 = -0.46f;
    highFilter.a2 = 0.23f;
    highFilter.b1 = -0.65f;
    highFilter.b2 = 0.25f;
    highFilter.gain = 0.8f; // Slightly reduce high freq to avoid harshness

    // De-essing filter (notch around 6-8kHz where sibilance occurs)
    // Improve the de-essing filter parameters for stronger S reduction
    deesingFilter.a0 = 0.98f;
    deesingFilter.a1 = -1.92f;
    deesingFilter.a2 = 0.98f;
    deesingFilter.b1 = -1.92f;
    deesingFilter.b2 = 0.94f;
    deesingFilter.gain = 0.85f; // Slightly reduce gain to attenuate sibilance more
    ...
    try {
        ...
        for (int i = 0; i < bufferSize; i++) {
            ...
            for (int ch = 0; ch < channels; ch++) {
                ...
                // Apply each EQ band and scale by the slider value (more gentle scaling)
                float bassOut = bassFilter.process(original) * (smoothBassEQ / 15.0f);
                
                // Apply bass boost if enabled (stronger bass effect separate from EQ)
                if (bassBoostEnabled) {
                    // Apply a more moderate bass boost with smooth transition
                    float moderateBass = bassFilter.process(bassFilter.process(original)) * 1.2f;
                    // Apply the transition smoothly to avoid clicks/pops
                    bassOut += (moderateBass + (tanh(moderateBass * 0.3f) * 0.4f)) * bassBoostTransition;
                }
                
                float midOut = midFilter.process(original) * (smoothMidEQ / 15.0f);

                // Apply de-essing before high frequencies to reduce sibilance
                float highPassed = highFilter.process(original);
                float deEssed = deesingFilter.process(highPassed); // De-ess the high frequencies
                float highOut = deEssed * (smoothHighEQ / 15.0f); // Gentler scaling for highs

                // Apply a more gentle mixing approach to preserve transients
                float eq_mix = 0.7f; // 70% processed, 30% original signal for clarity
                processedBuffer[idx] = original * (1.0f - eq_mix) + (original + bassOut + midOut + highOut) * eq_mix;

                // Apply a gentle limiter to prevent clipping from EQ
                if (processedBuffer[idx] > 0.95f) {
                    processedBuffer[idx] = 0.95f + (processedBuffer[idx] - 0.95f) * 0.05f;
                }
                else if (processedBuffer[idx] < -0.95f) {
                    processedBuffer[idx] = -0.95f + (processedBuffer[idx] + 0.95f) * 0.05f;
                }
            }
        }
        ...
        // Apply panning (for stereo only)
        if (channels == 2) {
            for (int i = 0; i < bufferSize; i++) {
                // Scale panningValue down to a reasonable range for the gain calculation
                // Convert from -10.0 to 10.0 range to a -1.0 to 1.0 range for the gain calculation
                float normalizedPanning = panningValue / 10.0f;
                
                // More pronounced panning effect
                float leftGain = (normalizedPanning <= 0) ? 1.0f : (1.0f - normalizedPanning * 0.8f);
                float rightGain = (normalizedPanning >= 0) ? 1.0f : (1.0f + normalizedPanning * 0.8f);
                ...
                if (leftIdx < bufferSize * channels && rightIdx < bufferSize * channels) {
                    ...
                    processedBuffer[leftIdx] *= leftGain;
                    processedBuffer[rightIdx] *= rightGain;
                }
            }
        }

        // Apply In-Head effects (for stereo only)
        if (channels == 2) {
            for (int i = 0; i < bufferSize; i++) {
                int leftIdx = i * 2;
                int rightIdx = i * 2 + 1;
                ...
                if (leftIdx < bufferSize * channels && rightIdx < bufferSize * channels) {
                    // In-Head Left: Boost left, reduce right
                    if (inHeadLeft) {
                        processedBuffer[leftIdx] *= 1.5f;
                        processedBuffer[rightIdx] *= 0.6f;
                    }

                    // In-Head Right: Boost right, reduce left
                    if (inHeadRight) {
                        processedBuffer[leftIdx] *= 0.6f;
                        processedBuffer[rightIdx] *= 1.5f;
                    }
                }
            }
        }
        ...
        if (energyEnabled) {
            ...
            float energyFactor = 1.0f + (energyValue / 750000.0f) * energyMod;
        ...
        for (int i = 0; i < bufferSize; i++) {
            ...
            for (int ch = 0; ch < channels; ch++) {
                ...
                // First apply a soft compression to prevent major peaks
                if (sample > 0.7f) {
                    float excess = sample - 0.7f;
                    sample = 0.7f + excess * 0.6f;
                }
                else if (sample < -0.7f) {
                ...
                // Apply gain after initial soft-knee and de-essing
                sample *= frameGain;

                // Special clarity enhancement for rage gain (when ExpGain is high)
                if (smoothExpGain > 50.0f) {
                    // Calculate clarity factor - increased with higher rage gain
                    float clarityFactor = Min(0.5f, (smoothExpGain - 50.0f) / 100.0f);
                    
                    // Presence boost - enhance mid frequencies for better speech intelligibility
                    float presenceBoost = midFilter.process(sample) * 0.3f * clarityFactor;
                    
                    // Apply subtle harmonic enhancement for clarity
                    float harmonic = tanh(sample * 0.6f) * 0.2f * clarityFactor;
                    
                    // Mix in the clarity enhancements
                    sample = sample * (1.0f - clarityFactor) + (sample + presenceBoost + harmonic) * clarityFactor;
                    
                    // Dynamic range adjustment to preserve transients
                    float attackSharpness = Min(1.0f, fabs(sample * 3.0f));
                    sample *= (0.85f + attackSharpness * 0.15f); // Preserve attack transients
                }

                // Apply a more aggressive limiter after gain to prevent distortion while preserving clarity
                if (sample > 0.9f) {
                    float excess = sample - 0.9f;
                    // Adaptive limiting for high gain values - preserves more detail for rage gain
                    float limitFactor;
                    if (smoothExpGain > 70.0f) {
                        // Special handling for rage gain to maintain clarity
                        // Two-stage limiting with softer curve
                        if (excess < 0.2f) {
                            limitFactor = 0.1f; // Preserve more detail in lower excess
                        } else {
                            limitFactor = 0.02f; // Hard limit higher excess
                        }
                    } else {
                        // Standard limiting for normal gain
                        limitFactor = totalGain > 100.0f ? 0.03f : (totalGain > 50.0f ? 0.08f : 0.15f);
                    }
                    sample = 0.9f + excess * limitFactor;
                }
                if (sample < -0.9f) {
                    float excess = -0.9f - sample;
                    // Adaptive limiting for high gain values - preserves more detail for rage gain
                    float limitFactor;
                    if (smoothExpGain > 70.0f) {
                        // Special handling for rage gain to maintain clarity
                ...
                // Final safety clamp to prevent any lingering extreme values
        ...
        if (g_eqEnabled) {
            ...
            for (int c = 0; c < channels; c++) {
                for (int i = 0; i < bufferSize; i++) {
                    float sample = audioBuffer[i * channels + c];
                    
                    // Apply EQ
                    float eqGain = 1.0f;
                    for (int band = 0; band < 10; band++) {
                        float freq = band == 0 ? 20.0f :
                                   band == 1 ? 60.0f :
                                   band == 2 ? 170.0f :
                                   band == 3 ? 310.0f :
                                   band == 4 ? 600.0f :
                                   ...
                                   band == 6 ? 3000.0f :
                                   band == 7 ? 6000.0f :
                                   band == 8 ? 12000.0f : 14000.0f;
                        
                        // Simple frequency-based gain adjustment
                        float gainDb = g_eqValues[band];
                        float gain = powf(10.0f, gainDb / 20.0f);
                        
                        // Apply gain based on frequency
                        eqGain *= gain;
                    }
                    
                    // Apply the EQ gain
                    audioBuffer[i * channels + c] = sample * eqGain;
                }
            }
        }
...
extern "C" opus_int32 custom_opus_encode(OpusEncoder* st, const opus_int16* pcm, int frame_size,
    ...
    try {
        ...
        // Process the audio with our effects
        ...
        opus_int16* processedPcm = new opus_int16[frame_size * channels];
        if (!processedPcm) {
            delete[] audioBuffer;
            return opus_encode(st, pcm, frame_size, data, max_data_bytes);
        }
                    ...
                    if (ImGui::BeginTabBar("MainTabBar", ImGuiTabBarFlags_FittingPolicyScroll)) {
                        ...
                        if (ImGui::BeginTabItem("Encoder")) {
                            ...
                            if (ImGui::IsItemHovered()) {
                                ...
                                ImGui::Bullet(); ImGui::TextUnformatted("Mono: Single channel, lower bandwidth usage");
                                ImGui::Bullet(); ImGui::TextUnformatted("Stereo: Dual channels, better audio quality");
                            ...
                            // Bass Boost checkbox centered similarly to the other checkboxes
                            ImGui::SetCursorPosX(encoderLeftMargin + encoderContentWidth/2 - 60);
                            ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(4, 3));
                            if (ImGui::Checkbox("Bass", &bassBoostEnabled)) {
                                // When toggling bass boost, apply immediately
                            }
                            ImGui::PopStyleVar();
                            if (ImGui::IsItemHovered()) {
                                ImGui::SetTooltip("Adds EXTREMELY powerful bass boost - use with caution!");
                            }
                            ...
                            DrawSlider("Bass", &bassEQ, 0.0f, 30.0f, "Boosts low frequencies");

                            // Mid EQ with tooltip
                            DrawSlider("Mid", &midEQ, 0.0f, 30.0f, "Boosts mid frequencies");

                            // High EQ with tooltip
                            DrawSlider("High", &highEQ, 0.0f, 30.0f, "Boosts high frequencies");
                            ...
                            // Use AnimatedPanningSlider for better visual indication
...
void AnimatedSlider(const char* label, float* value, float min, float max, const char* tooltip) {
    ...
    bool valueChanged = ImGui::SliderFloat("##slider", value, min, max, "");
...
bool AnimatedPanningSlider(const char* label, float* value, float min, float max) {
    ...
    ImGui::SetCursorPosX(leftPadding);
    ImGui::PushItemWidth(sliderWidth);
    bool valueChanged = ImGui::SliderFloat("##panning", value, min, max, "");
...
// Add this before the utilities::ui::start() function
void DrawEQGraph(const char* label, float* eqValues, int numBands, const ImVec2& size) {
    ...
    bool is_dragging = ImGui::IsMouseDragging(0);
    
    if ((is_clicked || is_dragging) && is_hovered) {
        float normalized_x = (mouse_pos.x - canvas_pos.x) / canvas_size.x;
        ...
        if (normalized_x >= 0.0f && normalized_x <= 1.0f && normalized_y >= 0.0f && normalized_y <= 1.0f) {
            int band_index = (int)(normalized_x * (numBands - 1) + 0.5f);
            if (band_index >= 0 && band_index < numBands) {
                float db_value = normalized_y * 24.0f - 12.0f; // Convert from 0-1 to -12 to +12 dB
                eqValues[band_index] = db_value;
            }
        }
    }
    ...
    const ImVec4 grid_color(0.5f, 0.5f, 0.5f, 0.2f);
    const ImVec4 text_color(0.7f, 0.7f, 0.7f, 1.0f);
    
    // Horizontal grid lines
    for (float y = canvas_pos.y; y < canvas_pos.y + canvas_size.y; y += grid_spacing) {
        draw_list->AddLine(
    ...
    // Vertical grid lines
    for (float x = canvas_pos.x; x < canvas_pos.x + canvas_size.x; x += grid_spacing) {
        draw_list->AddLine(
    ...
    const char* freq_labels[] = {"20Hz", "100Hz", "1kHz", "10kHz", "20kHz"};
    float label_positions[] = {0.0f, 0.25f, 0.5f, 0.75f, 1.0f};
    
    for (int i = 0; i < 5; i++) {
        float x = canvas_pos.x + label_positions[i] * canvas_size.x;
        draw_list->AddText(
            ImVec2(x - 15, canvas_pos.y + canvas_size.y + 5),
    ...
    // Draw dB labels
    const char* db_labels[] = {"+12dB", "+6dB", "0dB", "-6dB", "-12dB"};
    float db_positions[] = {0.0f, 0.25f, 0.5f, 0.75f, 1.0f};
    
    for (int i = 0; i < 5; i++) {
        float y = canvas_pos.y + db_positions[i] * canvas_size.y;
        draw_list->AddText(
            ImVec2(canvas_pos.x - 35, y - 7),
            ImGui::ColorConvertFloat4ToU32(text_color),
            db_labels[i]
        );
    }
    ...
    for (int i = 0; i < numBands; i++) {
        float x = canvas_pos.x + (float)i / (numBands - 1) * canvas_size.x;
        float normalized_value = (eqValues[i] + 12.0f) / 24.0f; // Convert from dB to 0-1 range
        float y = canvas_pos.y + (1.0f - normalized_value) * canvas_size.y;
        
        ImVec2 current_point(x, y);
        
        if (!first_point) {
            // Draw line with gradient
            ImVec4 line_color = ImVec4(1.0f, 0.0f, 0.0f, 1.0f); // Red color for the line
            draw_list->AddLine(
                prev_point,
                current_point,
